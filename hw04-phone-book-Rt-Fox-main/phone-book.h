...
//name, total call duration, number
struct  Comp_name {
  bool operator()(const user_info_t &a, const user_info_t &b) const;
};
//total call duration, name, number
struct  Comp_total {
  bool operator()(const user_info_t &a, const user_info_t &b) const;
};
...
private:
  ...
  //все контакты для быстрого доступа
  std::map<std::string, user_info_t> users;
  //search_users_by_name, префикс имя
  std::set<user_info_t, Comp_name> users_by_name;
  //search_users_by_number, префикс номер
  std::map<std::string,std::set<user_info_t, Comp_total>> users_by_total;
};
